...
class Text2CAD(nn.Module):
    ...
    def __init__(self, text_config, cad_config, tokenizer=None):  # 添加 tokenizer 参数
        super().__init__()

        # Base Text Embedder
        self.base_text_embedder = TextEmbedder.from_config(
            text_config['text_embedder'],
            tokenizer=tokenizer  # 传递分词器
        )
    ...
    def forward(
        ...
        ca_mask = {
            "attn_mask": prepare_cross_attention_mask_batch(key_padding_mask, cad_seq_len=self.cad_seq_len),
            "key_padding_mask": key_padding_mask
        }
